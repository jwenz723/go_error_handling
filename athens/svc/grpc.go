...
func encodeGRPCNewOrderResponse(_ context.Context, response interface{}) (interface{}, error) {
	const op = errors.Op("grpc.NewOrder")
	resp := response.(NewOrderResponse)
	return &pb.NewOrderReply{OrderID: resp.OrderID, Err: errors.E(op, resp.Err).Error()}, nil
}
...
func str2err(op errors.Op, s string) error {
	if s == "" {
		return nil
	}
	return errors.E(op, s)
}
...
func decodeGRPCNewOrderResponse(_ context.Context, grpcReply interface{}) (interface{}, error) {
	...
	return NewOrderResponse{OrderID: reply.OrderID, Err: str2err(errors.Op("grpc.NewOrder"), reply.Err)}, nil
}
